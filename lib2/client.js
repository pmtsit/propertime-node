...
var DEFAULTS = {
    reauth: false,
    sessionCookie: 'propertime_api_session',
    apiUrl: 'https://test-api.propertime.io/v1',
    reportsUrl: 'http://test.propertime.io'
};


module.exports = ProperTimeClient;
 ...
 */
function ProperTimeClient(options) {
    /** @private */
 ...
 * @static
 */
ProperTimeClient.USER_AGENT = 'propertime-node v' + VERSION;


/**
 * @public
 * @static
 * @returns {ProperTimeClient}
 */
ProperTimeClient.defaultClient = function defaultClient() {
    if (!DEFAULT_INSTANCE) {
        DEFAULT_INSTANCE = new ProperTimeClient();
    }
 ...
 */
ProperTimeClient.setDefaults = function setDefaults(newDefaults) {
 ...
 */
ProperTimeClient.prototype.apiRequest = function apiRequest(path, opts, callback) {
    var self = this;
    ...
    return fetch(opts.url,
        ...
        .then((res) => {
            ...
            var statusCode = result.res.status;

            if (statusCode >= 200 && statusCode < 300) {
                return callback(null, result.data);
            }
            else {
                return callback(new APIError(statusCode, result.data));
            }
...
/**
 * @see https://github.com/pmtsit/propertime_api_docs/blob/master/chapters/authentication.md
 * @public
 * @param {Function} [callback] <code>(err, userData)</code>
 */
ProperTimeClient.prototype.authenticate = function authenticate(callback) {
    var self = this;
    var options = this.options;
    var auth;
    var req = {};
...
/** @public */
ProperTimeClient.prototype.destroy = function destroy() {
    if (this.authTimeout) {
 ...
 * @param {Function} callback <code>(err, data)</code>
 */
ProperTimeClient.prototype.reportsRequest = function reportsRequest(path, opts,
                                                               callback) {
    var options = this.options;
    ...
    opts.json = true;
    opts.qs = opts.qs || {};
    opts.qs.user_agent = ProperTimeClient.USER_AGENT;
 ...
 */
ProperTimeClient.prototype.setAuthTimer = function setAuthTimer(duration) {
    var self = this;
 ...
 * @returns {Boolean}
 */
ProperTimeClient.prototype.validateOptions = function validateOptions(schema,
                                                                 options, callback) {
...
function validateOptions(options) {
    ...
    if (!options.apiUrl) {
        throw new Error('ProperTime API base URL is not specified');
    }
}
